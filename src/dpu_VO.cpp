  ...
  while (n.ok())
  {
    ...
    if ( (! _this->frame_queue.empty())){
        ...
        if ( (! running_dpu_NetVLAD) && ( (depth_cpu_NetVLAD <= 1 && running_cpu_NetVLAD) || (depth_cpu_NetVLAD < 1 && ! running_cpu_NetVLAD) ) ){
            ...
            ROS_INFO_STREAM("Queue depth: " << _this->frame_queue.size() << "; WORD: " << _this->frame_queue.front()->data.c_str() );
            // starting time
            ros::Time start = ros::Time::now();
            // doing computation
            usleep(3*1000);
            // finishing time
            ROS_INFO_STREAM("image ID: " << _this->frame_queue.front()->data.substr(12) << "; starting time: " << start << "; finishing time: " << ros::Time::now());
            std::stringstream ss;
            std_msgs::String msg;
